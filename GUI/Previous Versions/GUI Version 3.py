...
def main():
    ...
    pygame.display.set_caption('AI Drawing Board')
    ...
    # Canvas
    CANVAS_LOGICAL = 128
    # Scale up for display but keep center of screen
    ...
    #Brush slider in settings box at bottom right

    SETTINGS_H = 110
    set_y = SH - SETTINGS_H - 16
    brush_slider = Slider(RIGHT_X + 20, set_y + 60, RIGHT_PANEL_W - 40, 1, 20, 3, 'Brush Size')

    #Left panel (Class Examples)
    LEFT_PANEL_W = 250
    CLOSED_X = -LEFT_PANEL_W + 28   # tab sticks out
    OPEN_X   = 0
    ...
    while True:
        ...
        lpx = int(panel_anim)   # left panel x
        ...
        if drawing:
            ...
            if c_rect.collidepoint(mouse):
                ...
                if last_lp:
                    cv2.line(canvas_np, last_lp, lp, (0, 0, 0), br * 2)
                else:
        ...
        #canvas label
        clbl = font_sm.render(f'128 × 128 px canvas', True, TEXT_DIM)
        ...
        screen.blit(tb, (0, 0))
        tt = font_title.render('✏  AI Drawing Board', True, TEXT_PRI)
        screen.blit(tt, (SW // 2 - tt.get_width() // 2, 9))
        ht = font_sm.render('ESC / Q  quit', True, TEXT_DIM)
